...
public class GattClientCallback extends BluetoothGattCallback {
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        ...
        if (status == BluetoothGatt.GATT_FAILURE) {
            ...
            mClientActionListener.logError("Connection not GATT success status " + status);
            mClientActionListener.disconnectGattServer();
            return;
        }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        //Fehler tritt hier auf .. matchingCharacteristics ist leer .. gatt ist fehlerhaft
        List<BluetoothGattCharacteristic> matchingCharacteristics = BluetoothUtils.findCharacteristics(gatt);
        if (matchingCharacteristics.isEmpty()) {
            mClientActionListener.logError("Unable to find characteristics. Im the error" + status+ BluetoothGatt.GATT_SUCCESS+ BluetoothGatt.GATT_FAILURE+ BluetoothProfile.STATE_CONNECTED );
