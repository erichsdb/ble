...
public class GattServerCallback extends BluetoothGattServerCallback {
    ...
    public void onCharacteristicWriteRequest(BluetoothDevice device,
        ...
        if (CHARACTERISTIC_ECHO_UUID.equals(characteristic.getUuid())) {
            ...
            if(date.length() > 18){
                newDate = date.substring(0,18);
            }
            //Response to value array
            byte[] response;
            //If Input is 0 (0030) response is the date
            if (value[0] == 0x30){
                response = StringUtils.bytesFromString(newDate);
                characteristic.setValue(response);
                mServerActionListener.notifyCharacteristicEcho(response);
            }else{
                //else it's just the reverse input
                response = ByteUtils.reverse(value);
                characteristic.setValue(response);
                mServerActionListener.notifyCharacteristicEcho(response);
            }

            mServerActionListener.log("Current Date: " + newDate);
